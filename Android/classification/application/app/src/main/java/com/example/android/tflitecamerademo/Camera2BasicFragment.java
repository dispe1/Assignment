...
public class Camera2BasicFragment extends Fragment
  ...
  private void classifyFrame() {
    ...
    if(!faceDetector.isOperational()){
      showToast("Face Detector Not build!");
      return;
    }
    ...
    else
    {
        ...
        float x1 = thisFace.getPosition().x + (float) 0.12*thisFace.getWidth();
        float y1 = thisFace.getPosition().y + thisFace.getHeight() - (float)0.76*thisFace.getWidth();
        float x2 = x1 + (float)0.76*thisFace.getWidth();
        float y2 = y1 + (float)0.76*thisFace.getWidth();
        int length = (int)(0.76*thisFace.getWidth());

        // 얼굴 크로핑 후 128픽셀로 변환
      if(x1 > 0 && x1 + length <= bitmap.getWidth() && y1 > 0 && y1 + length <= bitmap.getHeight()) {
        Bitmap cropFace = Bitmap.createBitmap(bitmap, (int) x1, (int) y1, length, length);
        cropFace = createScaledBitmap(cropFace, 128, 128);

        String textToShow = classifier.classifyFrame(cropFace);
        bitmap.recycle();
        cropFace.recycle();
        showToast(textToShow);
        DrawFocusRect(x1, y1, x1 + length, y1 + length);
      }
      else {
        showToast("No face detected");
        DrawFocusRect(0, 0, 0, 0);
